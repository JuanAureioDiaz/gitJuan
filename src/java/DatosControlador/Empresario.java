    ...
    */
    ...
    public String NuevoEmpresario(){
        //Manda llamar el método conectar
          ConectarBD();
        try{
            String registrarE="insert into empresario values(0,?,?,?,?,?,?, now())";
    ...
    public String ConsultaEmpresarios(){
        ...
        try{
            ...
            while(datosConsultados.next()){
                ...
                emp.setNombreUsr(datosConsultados.getString(nombreUsr));
                emp.setApaternoUsr(datosConsultados.getString(apaternoUsr));
                emp.setAmaternoUsr(datosConsultados.getString(amaternoUsr));
                emp.setTelefonoUsr(datosConsultados.getString(telefonoUsr));
                emp.setCorreoUsr(datosConsultados.getString(correoUsr));
    ...
    public String DetalleEmpresario(){
        ConectarBD();
        try{
            String consultaDE="select nickEmpresario,nombre, apaterno, amaterno, correo, telefono, fregistro"
                    + "from Empresario where nickEmpresario='"+nickUsr+"'";
            ...
            while(RDEmpresario.next()){
                setNombreUsr(RDEmpresario.getString(nombreUsr));
                setApaternoUsr(RDEmpresario.getString(apaternoUsr));
                setAmaternoUsr(RDEmpresario.getString(amaternoUsr));
                setCorreoUsr(RDEmpresario.getString(correoUsr));
                setTelefonoUsr(RDEmpresario.getString(telefonoUsr));
    ...
    public String ModificaEmpresario(){
        ...
        ConectarBD();
        try{
            String modEmpresario= "update empresario set nombreUsr=?, apaternoUsr=?, amaternoUsr=?"
                    + ",telefonoUsr=?, correoUsr=?, passwdUsr=? where nickUsr=? ";
